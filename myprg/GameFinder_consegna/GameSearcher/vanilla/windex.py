...
class FileManager:
    ...
    def __init__(self, package_name , condition):
        ...
        if condition == "b" : # dati di benchmark riferimenti booleani
...
class SchemaManager(FileManager):
  ...
  """
  schema = Schema (  
      title=TEXT (stored=True), # titolo del gioco 
  ...
  def _preprocessing(self,stringa):
      ...
      stringa_token = nltk.word_tokenize(stringa) # effettua la tokenizzazione
      stringa_token = [ word.lower() for word in stringa_token if word.isalpha()] # toglie maiuscole e tiene solo caratteri alfanumerici
      stringa_token = [word for word in stringa_token if word not in stopwords.words("english")] # toglie le stopwords
