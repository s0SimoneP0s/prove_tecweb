...
class Search(SchemaManager):
  ...
  def __init__(self , package="sentiment_vanilla" , condition="b" ,advanced=False,  pre=True,  limit=100 ):
  ...
  def definitive_score(self, whoosh_score, sentiment_score, ww=0.4 , sw=1 , pos=True):
    """Bilanciamento per la sentiment ananlisys rispetto ai valori di whoosh"""
    whoosh_weight=ww
    sentiment_weight=sw
    #if whoosh_score == 0 and sentiment_score == 0:
    #  return 0
    #elif sentiment_score == 0:
    #  return whoosh_score
    #elif whoosh_score*==0:
    #  return sentiment_score
    if not pos: # inverte positività sentiment
       sentiment_score=1-sentiment_score
    return whoosh_weight * whoosh_score + sentiment_weight * (sentiment_score - 0.5 )
  ...
  def _title_sorting(self, results):
      ...
      for i in results:
          ...
          r.append(i) # aggiunge alla lista di ritorno
      grouped_results = {} 
      l=len(r)
      ...
      for result in results:
          ...
          # per la sentiment
          sentiment_score=result['rank']
          score = self.definitive_score(score, sentiment_score)
      ...
      grouped_results2 = {} # calcola la media delle recensioni per ogni titolo
      ...
      avg_map = sum_avfg / l if l > 0 else 0  # media tra tutti i documenti unici
  ...
  def wu_palmer_similarity(self, word1, word2):
    """Usa la wupalmer similarity per la distanza tra word1 e word2"""
  ...
  def __search_backend(self, query_str ):
    ...
    with self.ix.searcher(weighting=self.scorer) as searcher:
      # nostri algoritmi personalizzati
      if self.advanced :
        query = QueryParser("review", self.ix.schema).parse(query_str ) # supporta wildcard, boolean e proximity
      else:
        query = self.synonymous_expansion(query_str)  
